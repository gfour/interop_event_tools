...
if [ "$1" == "" ]; then
    echo "Usage: ./create-issuer-cert.sh <DOMAIN>"
    echo "Creates a certificate for issuance"
    exit
fi
...
CRT="$1.crt"
openssl x509 -req -in ${CSR} -CA root-ca-grnet.pem -CAkey grnet.key -CAcreateserial -out ${CRT} -days 400 -sha256 -extfile eudi-cert.conf -extensions v3_ca
